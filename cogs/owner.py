    ...
    async def restart(self, ctx):
        ...
        async def callback(int: discord.Interaction):
            if int.user != ctx.author:
                await int.response.send_message(f"<:zzcross64:1189187617540022392> | It's not your interaction.")
                return
        ...
        async def callback2(int: discord.Interaction):
            if int.user != ctx.author:
                await int.response.send_message(f"<:zzcross64:1189187617540022392> | It's not your interaction.", ephemeral=True)
                return
