...
class fun(commands.Cog):
    ...
    async def quickpoll_cmd(self, ctx: commands.Context, *, question: str): 
      message = await ctx.reply(embed=discord.Embed(color=0x7289da, description=question).set_author(name=f"{ctx.author} asked"))
    ...
    async def rockpaperscisssors(self, ctx: commands.Context): 
      ...
      embed = discord.Embed(color=0x7289da, title="Rock Paper Scissors!", description="Click a button to play!")
    ...
    async def howretarded(self, ctx, member: discord.Member=commands.Author):
     await ctx.reply(embed=discord.Embed(color=0x7289da, title="how retarded", description=f"{member.mention} is {randrange(101)}% retarded <:blue_dumb:1092786568974049341>"))
    ...
    async def howgay(self, ctx, member: discord.Member=commands.Author):
     await ctx.reply(embed=discord.Embed(color=0x7289da, title="gay r8", description=f"{member.mention} is {randrange(101)}% gay 🏳️‍🌈"))
    ...
    async def howcool(self, ctx, member: discord.Member=commands.Author):
     await ctx.reply(embed=discord.Embed(color=0x7289da, title="cool r8", description=f"{member.mention} is {randrange(101)}% cool 😎"))
    ...
    async def iq(self, ctx, member: discord.Member=commands.Author):
     await ctx.reply(embed=discord.Embed(color=0x7289da, title="iq test", description=f"{member.mention} has `{randrange(200)}` iq 🧠"))
    ...
    async def hot(self, ctx, member: discord.Member=commands.Author):
     await ctx.reply(embed=discord.Embed(color=0x7289da, title="hot r8", description=f"{member.mention} is `{randrange(100)}%` hot 🥵"))     
    
    @commands.hybrid_command()
    async def pp(self, ctx:commands.Context, *, member: discord.Member=commands.Author):
      lol = "===================="
      embed = discord.Embed(color=0x7289da, description=f"{member.name}'s penis\n\n8{lol[random.randint(1, 20):]}D")
      await ctx.reply(embed=embed)  

    @commands.command(name='timer')
    async def timer(self, ctx, time: str):
        """Sets a timer for a specified number of hours, minutes, or seconds."""
        unit = time[-1]
        if not time[:-1].isdigit():
            await ctx.send("Please provide a valid number followed by 'h', 'm', or 's'.")
            return
        ...
        if unit == 'h':
            ...
            await ctx.send("Please use 'h' for hours, 'm' for minutes, or 's' for seconds.")
            return

        if total_seconds <= 0:
            await ctx.send("You need to set a timer for at least 1 second.")
            return
        
        await ctx.send(f'Timer set for {duration} {unit_text}.')
        await asyncio.sleep(total_seconds)
        await ctx.send(f'{ctx.author.mention}, your timer for {duration} {unit_text} is up!')
    ...
    async def bitches(self, ctx: commands.Context, *, user: discord.Member=commands.Author):
      ...
      embed = discord.Embed(color=0x7289da, description=f"{user.mention} has `{result}` bitches")
      await ctx.reply(embed=embed)
    ...
    async def urban(self, ctx, *, word):
      ...
      try:
       ...
       defs = data["list"]
       for defi in defs: 
        e = discord.Embed(color=0x7289da, description=defi["definition"], timestamp=dateutil.parser.parse(defi["written_on"]))
    ...
    async def tictactoe(self, ctx: commands.Context, *, member: discord.Member):
      if member is ctx.author: return await ctx.reply(embed=discord.Embed(color=0x6d827d, description=f"{self.bot.warning} {ctx.author.mention}: You can't play with yourself. It's ridiculous"))
      if member.bot: return await ctx.reply("bots can't play")      
      embed = discord.Embed(color=0x7289da, description=f"**{ctx.author.name}** wants to play **tictactoe** with you. Do you accept?")
      style = discord.ButtonStyle.gray
      yes = discord.ui.Button(emoji='<:ticky:1154027584020021278>', style=style)
      no = discord.ui.Button(emoji='<:crosss:1154027599933222954>', style=style)
      
      async def yes_callback(interaction: discord.Interaction): 
         if interaction.user != member:
           em = discord.Embed(color=0x7289da, description=f"{self.bot.warning}: {interaction.user.mention} you are not the author of this embed")
      ...
      async def no_callback(interaction: discord.Interaction): 
         if interaction.user != member:
           em = discord.Embed(color=0x7289da, description=f"{self.bot.warning}: {interaction.user.mention} you are not the author of this embed")
           return await interaction.response.send_message(embed=em, ephemeral=True)
         await interaction.response.edit_message(embed=discord.Embed(color=0x7289da, description=f"I'm sorry but **{interaction.user.name}** doesn't want to play with you right now"), view=None, content=ctx.author.mention)
    ...
    async def blacktea(self, ctx: commands.Context): 
     ...
     embed = discord.Embed(color=0x7289da, title="BlackTea Matchmaking", description=f"⏰ Waiting for players to join. To join react with 🍵.\nThe game will begin in **20 seconds**")
     ...
     for leader in leaderboard[::-1]: 
      auto += f"{'<:880765863953858601:1152979775321812992>' if le == 1 else f'`{le}`'} **{ctx.guild.get_member(leader) or leader}**\n"
      if le == 10: break
      le += 1
     e = discord.Embed(color=0x6d827d, title=f"leaderboard for blacktea", description=auto).set_footer(text=f"top {'10' if len(leaderboard) > 9 else len(leaderboard)} players")     
    ...
    async def typerace(self, ctx: commands.Context):
     ...
     except asyncio.TimeoutError: return await ctx.reply(embed=discord.Embed(color=0x7289da, description="🙁 {} you took too long to reply".format(ctx.author.mention))) 
    
     if guess.content == mam: 
        fintime = time.time()
        total = fintime - startTime
        embed = discord.Embed(color=0x7289da, title="Congratulations!", description=f"You typed the message perfectly (100% accuracy) in {total:.2f} seconds")
        return await guess.reply(embed=embed)
     else: 
      fintime = time.time()
      total = fintime - startTime  
      embed = discord.Embed(color=0x7289da, description="You typed the sentence in **{}** seconds with an accuracy of **{}%**".format(f"{total:.2f}", TypeRace.accurate(guess.content, mam)))
      return await guess.reply(embed=embed)
    ...
    async def mtball(self, ctx: commands.Context, *, arg):      
     ...
     e = discord.Embed(color=0x7289da, description=f"You asked: {arg}\nAnswer: {random.choice(rand)}")
