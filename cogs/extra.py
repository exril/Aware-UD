...
class extra(commands.Cog):
    ...
    async def _enable(self, ctx, *, perms):
        ...
        if not ctx.guild.me.guild_permissions.administrator:
            return await ctx.reply(embed=discord.Embed(description="Gateway lacks certain permissions to run this function properly kindly refer to [our documentation for more](https://docs.awarebot.pro/faqs/#gateway-lacks-certain-permissions-to-run-this-function-properly)", color=0xc283fe))
    ...
    async def _disable(self, ctx):
        ...
        if ctx.guild.owner.id == ctx.author.id:
            ...
            if ctx.author.top_role.position <= ctx.guild.me.top_role.position and ctx.author.id not in  [978930369392951366, 979353019235840000]:
                ...
                return await ctx.send(embed=em)
        if not ctx.guild.me.guild_permissions.administrator:
            return await ctx.reply(embed=discord.Embed(description="Gatewaylacks certain permissions to run this function properly kindly refer to [our documentation for more](https://docs.awarebot.pro/faqs/#gateway-lacks-certain-permissions-to-run-this-function-properly)", color=0xc283fe))
        with sqlite3.connect('database.sqlite3') as db1:
            db1.row_factory = sqlite3.Row
            cursor1 = db1.cursor()
            cursor1.execute(f"SELECT * FROM prefixes WHERE guild_id = {ctx.guild.id}")
