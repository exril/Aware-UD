...
class moderation(commands.Cog):
    ...
    async def delemoji(self, ctx, emoji = None):
        init = await ctx.reply(f"<a:loading:1200297027028918322> Processing the command...", mention_author=False)
        con = None
    ...
    async def role_all(self, ctx, *,role: discord.Role):
        ...
        em=discord.Embed(description=f"**<a:loading:1200311463584219196>  |  Giving __{role.mention}__ to {len(test)} Members**", color=ctx.author.color)
    ...
    async def role_bots(self, ctx, *,role: discord.Role):
        ...
        if v.value == 'Yes':
            ...
            return await init.delete()
        self.bot.role_status[ctx.guild.id] = (0, len(test), True)
        em=discord.Embed(description=f"**<a:loading:1200311463584219196>  |  Giving __{role.mention}__ to {len(set(test))} Bots**", color=ctx.author.color)
        await init.edit(embed=em, view=None)
    ...
    async def role_humans(self, ctx, *,role: discord.Role):
        ...
        if v.value == 'Yes':
            ...
            return await init.delete()
        self.bot.role_status[ctx.guild.id] = (0, len(test), True)
        em=discord.Embed(description=f"**<a:loading:1200311463584219196>  |  Giving __{role.mention}__ to {len(set(test))} Users**", color=ctx.author.color)
        await init.edit(embed=em, view=None)
        for humans in test:
            if self.bot.role_status[ctx.guild.id] is not None:
                count, total_count, sts = self.bot.role_status[ctx.guild.id]
    ...
    async def rrole_all(self, ctx, *,role: discord.Role):
        ...
        em=discord.Embed(description=f"**<a:loading:1200297027028918322>  |  Removing __{role.mention}__ from {len(test)} Members**", color=ctx.author.color)
        await init.edit(embed=em, view=None)
        for member in test:
            if self.bot.rrole_status[ctx.guild.id] is not None:
    ...
    async def rrole_bots(self, ctx, *,role: discord.Role):
        ...
        if v.value == 'Yes':
            ...
            return await init.delete()
        self.bot.rrole_status[ctx.guild.id] = (0, len(test), True)
        em=discord.Embed(description=f"**<a:loading:1200297027028918322>>  |  Removing __{role.mention}__ from {len(set(test))} Bots**", color=ctx.author.color)
        await init.edit(embed=em, view=None)
        for bot_members in test:
            if self.bot.rrole_status[ctx.guild.id] is not None:
                count, total_count, sts = self.bot.rrole_status[ctx.guild.id]
