    ...
    async def spotifytrack(self, ctx, user: discord.Member = None):
        ...
        if user == ctx.author:
                if spotify_result is None:
                    embed = discord.Embed(description=f"{Emojis.spotify_emote} {ctx.author.mention}: **You are not currently listening to Spotify.**", color=Colors.spotify)
        ...
        if user == user:
            if spotify_result is None:
                embed = discord.Embed(description=f"{Emojis.spotify_emote} {ctx.author.mention}: **They are not currently listening to Spotify.**", color=Colors.spotify)
                ...
                report = await praser.file(report, "screw.png")
                ...
                embed.set_author(name=f"{user.name} is listening to:", icon_url=f"{user.display_avatar}")
                embed.set_thumbnail(url=f'{spotify_result.album_cover_url}')
                embed.set_image(url="attachment://screw.png")
                embed.set_footer(icon_url="https://freeimage.host/i/HMt8JIa", text=f"category: spotify・aware ©️ 2024")
