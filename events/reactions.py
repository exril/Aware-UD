...
class Reactions(commands.Cog): 
  ...
  async def starboard_add(self, payload: discord.RawReactionActionEvent):
     try:   
       ...
       if check: 
         ...
         chec = await self.self.bot.pg_conn.fetchrow(f"SELECT * FROM starboardmes WHERE guild_id = {payload.guild_id} AND channel_message_id = {payload.channel_id} AND message_id = {payload.message_id}") 
         if chec is not None: 
            ...
            if not message.embeds: embed = discord.Embed(color=0x6d827d, description=message.content, timestamp=message.created_at.now())
            else: 
            ...
            async def idk(): 
                  ...
                  await self.self.bot.pg_conn.execute("INSERT INTO starboardmes VALUES ($1,$2,$3,$4,$5)", payload.guild_id, channel.id, payload.channel_id, mes.id, payload.message_id)
  ...
  async def on_raw_reaction_remove(self, payload: discord.RawReactionActionEvent): 
   ...
   await self.self.bot.pg_conn.execute("INSERT INTO reactionsnipe VALUES ($1,$2,$3,$4,$5,$6,$7)", guild.id, payload.channel_id, member.name, member.display_avatar.url, payload.emoji.name, payload.emoji.url, payload.message_id)  
