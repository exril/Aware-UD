...
async def quick_link(
    ...
    embed = discord.Embed(
        ...
        description=(
            f"[Click here to view the transcript](https://tickets.gatewaybot.xyz/ticket?uri={message.attachments[0].url})"
        ),
        colour=discord.Colour.blurple(),
    )
...
async def link(
    ...
    :param message: discord.Message
    :return: string (link: https://tickets.gatewaybot.xyz/ticket?uri=ATTACHMENT_URL)
    """
    return "https://tickets.gatewaybot.xyz/ticket?uri=" + message.attachments[0].url
