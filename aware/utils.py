 ...
 async def invoke_cmds(ctx: commands.Context, member: Union[discord.Member, discord.User], embed: str) -> discord.Message:
  ...
  if res:
   ...
   if embed == "none": 
    ...
    return await ctx.send( f"Deleted the **{ctx.command.name}** custom response")
   elif embed == "view": 
    ...
    return await ctx.reply(embed=em)
      ...
      return await ctx.send( f"Updated your custom **{ctx.command.name}** message to {'the embed' if '--embed' in embed else ''}\n```{embed}```")
  else: 
   await ctx.bot.db.execute("INSERT INTO invoke VALUES ($1,$2,$3)", ctx.guild.id, ctx.command.name, embed)
   return await ctx.send( f"Added your custom **{ctx.command.name}** message to {'the embed' if '--embed' in embed else ''}\n```{embed}```")
